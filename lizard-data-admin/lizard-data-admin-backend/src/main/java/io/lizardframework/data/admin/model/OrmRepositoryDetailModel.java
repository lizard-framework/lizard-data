 ...
 */
@Data
public class OrmRepositoryDetailModel {
	/**
	 ...
	 */
	private List<OrmAtomDetailModel> atoms;

	public OrmRepositoryDetailModel(RepositoryDataSourceModel model) {
		this.setRepositoryName(model.getRepositoryName());
		this.setState(model.getState());
		this.setLoadBalance(model.getLoadBalance());
		this.atoms = model.getAtoms().stream().map(atom -> new OrmAtomDetailModel(atom)).collect(Collectors.toList());
	}
}
